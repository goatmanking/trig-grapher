...
angle_text_rect = pygame.Rect(200, 200, 142, 32)
period_text_rect = pygame.Rect(500, 400, 142, 32)
amplitude_text_rect = pygame.Rect(200, 600, 142, 32)
b_text_rect = pygame.Rect(200, 400, 142, 32)
        ...
        for i in range(angle):
            ...
            for i in range(angle):
                ...
                if cos_value > 0 or cos_value ==  math.cos(math.radians(0)):
                    pygame.draw.line(screen, 'green',(prev_x, prev_y), ((i * ratio_of_angle) + 40, h/2 -(cos_value * h/2)))
                    prev_x = (i * ratio_of_angle) + 40
                    prev_y = h/2 -(cos_value * h/2)
                elif cos_value < 0:
                    pygame.draw.line(screen, 'red',(prev_x, prev_y), ((i * ratio_of_angle) + 40,  h/2 -(cos_value * h/2)))
                    prev_x = (i * ratio_of_angle) + 40
                    prev_y = h/2 -(cos_value * h/2)
    elif func == 't':
            prev_x  = 40
...
angle_font =  font2.render(f'{angle_text}' ,False, (125, 0, 125))
period_font =  font2.render(f'{period_text}' ,False,  (125, 0, 125))
amplitude_font =  font2.render(f'{amplitude_text}' ,False,  (125, 0, 125))
b_font =  font2.render(f'{b_text}' ,False,  (125, 0, 125))

a_font =  font2.render(f'Angle: ' ,False, 'black')
p_font =  font2.render(f'Period: ' ,False,  'black')
amp_font =  font2.render(f'Amplitude:' ,False,  'black')
ba_font =  font2.render(f'B-value(use 0 if none): ' ,False, 'black')
...
while running:
    ...
    if trig_graph_started:
        ...
        screen.blit(a_font, (angle_text_rect.x - 20, angle_text_rect.y))
        ...
        screen.blit(p_font, (period_text_rect.x - 20, period_text_rect.y))
        ...
        screen.blit(amp_font, (amplitude_text_rect.x - 20, amplitude_text_rect.y))

        if (b_font.get_width() + 30) >= b_text_rect.w:  b_text_rect.w = b_font.get_width() + 30
        pygame.draw.rect(screen, 'white', b_text_rect, 0, 2)
        screen.blit(ba_font, (b_text_rect.x - 20, b_text_rect.y))
